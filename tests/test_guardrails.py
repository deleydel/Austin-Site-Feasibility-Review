...
def test_citation_rejects_unapproved_source():
    result = verify_citation(
        ...
        get_section=lambda s, n: {"status": "found", "matches": [{}]},
    )
...
def test_privacy_scrub_removes_contact_info():
    ...
    assert scrub_value({"note": "email me at a@b.com"})["note"].endswith(
        "[REDACTED]"
    ) or "[REDACTED]" in scrub_value({"note": "email me at a@b.com"})["note"]

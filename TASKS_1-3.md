...
`evaluation/`, and generated evidence in per-area `evaluation/*/results/` folders — these verify Tasks 1–3
...
Details for each number are below; raw outputs are in the `results/` folder of each `evaluation/` area.
...
(full detail incl. per-question retrievals: `evaluation/retrieval/results/retrieval_benchmark.md`):
...
`evaluation/tools/results/tool_latency_ms.json`; observations, not guarantees):
...
measurement. Each writes its results to its own `results/` subfolder
(`evaluation/retrieval/results/`, `evaluation/tools/results/`).

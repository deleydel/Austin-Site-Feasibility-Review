...
How this repo is organized and where each workstream's pieces live:
...
│
├── src/               # implementation code, one package per workstream
│   ├── config.py      #   shared paths & parameters — import this, don't hardcode paths
│   ├── preprocessing/ #   Task 1: raw data -> cleaned Parquet + manifest + QA report
│   ├── rag/           #   Task 2: DOCX -> chunks -> vector index -> hybrid retriever
│   └── tools/         #   Task 3: zoning / geocode / floodplain / watershed / nearby lookups
│                      #   (later workstreams — agents, guardrails, report, UI — add
│                      #    their own packages here alongside these)
│
├── tests/             # pytest correctness tests for Tasks 1–3 (run: python -m pytest tests/)
├── evaluation/        # development benchmarks (retrieval quality, tool latency)
├── reports/           # generated evidence: data-quality report, benchmark results
│
└── data/              # NOT in git (too large for GitHub) — only data/README.md is committed.
    ├── raw/           #   original DOCX/CSV/GeoJSON — download link in data/README.md
    ├── processed/     #   generated by  python -m src.preprocessing.run_all
    └── index/         #   generated by  python -m src.rag.indexer
```

**Getting started:** clone, install requirements, follow `data/README.md` to
download the raw data into `data/raw/`, then run the two generation commands
above (preprocessing, then indexer). `TASKS_1-3.md` documents everything the
existing code does and the results it achieves.

**Conventions:** everything runs as a module from the repo root
(`python -m src...`, `python -m evaluation...`); paths come from
`src/config.py`; generated artifacts go to `data/processed`, `data/index`, or
`reports/` — never commit anything under `data/` except its README.
